...
public class TxFetcher {
    ...
    public static Tx fetch(String txId, boolean testnet, boolean fresh) throws IOException, InterruptedException {
        if (fresh || !cache.containsKey(txId)) {
            ...
            if ((raw[4] & 0xFF) == 0) {
                ...
                tx = Tx.parse(new ByteArrayInputStream(trimmed), testnet);
                byte[] lockTimeBytes = Arrays.copyOfRange(raw, raw.length - 4, raw.length);
                tx.setLockTime(Helper.littleEndianToInt(lockTimeBytes).longValue());
            } else {
                tx = Tx.parse(new ByteArrayInputStream(raw), testnet);
            }
